...
평가 파일이 통과하더라도 공개 전에 서로 다른 두 명 이상의 현재 자격 확인된 임상의가 `POST /api/governance/model-release-reviews`에서 출력 안전성, 소크라테스 방식, 지연, 교육 전용 한계를 각각 확인해 승인 기록을 남겨야 합니다. 같은 임상의의 중복 승인은 한 명으로만 계산되며, 자격이 만료·정지되면 해당 승인은 즉시 공개 요건에서 제외됩니다. 운영 `/api/governance/readiness`와 학습 세션 시작/스트리밍은 필요한 독립 승인 수가 충족되지 않으면 차단됩니다.
